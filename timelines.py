...
def timeline(text,names,windowsize=1000):
    ...
    locs = [int((i.end()-i.start())/2)+i.start() for i in re.finditer(vocab_tools.vocab_regex(names),text)]
    if windowsize%2==0: windowsize = max(0,windowsize-1)
    radius = (windowsize-1)/2
    textlen = len(text)
    vector = np.zeros(textlen)
    
    # place into vector
    for i in locs: vector[int(max(0,i-radius)):int(min(i+radius+1,textlen))]+=1
    return vector
...
def plot_timeline(entity_dict,entity):
    ...
    """
    # initialize
    if isinstance(entity,str): entity = [entity]
    entity = [i for i in entity if i in entity_dict]
    if not entity:
        for i in entity_dict: print(i)
        print('Please choose from the above list of entities. Aborting.')
        return
    
    # plot
    plt.figure(figsize=(12,3))
    for i in entity: plt.plot(range(len(entity_dict[i])),entity_dict[i],label=i)
    plt.xlabel('Index')
