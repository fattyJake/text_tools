...
def perc_length_difference(textA,textB):
    ...
    """
    if len(textA)==0 and len(textB)==0: return 0
    if len(textA)==0 or len(textB)==0: return 1
    return abs(len(textA)-len(textB))/float(max(len(textA),len(textB)))

def unique_by_jaccard(texts,threshold=.95):
    """
    ...
    while i<len(texts)-1:
        ...
        while j<len(texts):
            ...
            # IF length_diff<=5% difference AND jaccard high, drop second version
            if perc_length_difference(texts[i],texts[j])<=0.05:
                
                try: # this try statement needs to be dropped. FIGURE IT OUT.
                    if len(tokens[i].intersection(tokens[j]))/float(len(tokens[i]|tokens[j]))>=threshold:
                        del texts[j]
                        del tokens[j]
                        continue
                except: pass
...
def repeated_find(texts,min_cluster_size=1):
    ...
    """
    # initialization
    if not isinstance(texts,list) or len(texts)<=min_cluster_size: return []
    ...
    while i<len(texts)-1:
        if len(texts[i])!=0 and i not in repeat_pages:
            j=i+1
            while j<len(texts):
                if j not in repeat_pages and len(texts[j])!=0 and texts[i]==texts[j]:
                    out.append((j,i))
    ...
    if min_cluster_size>1:
        ...
        while i<len(out)-1:
            j=i+1
            while j<len(out):
                if out[j][0]==out[j-1][0]+1 and out[j][1]==out[j-1][1]+1: j+=1
                else: break 
            if j-i>=min_cluster_size: acceptable = acceptable + out[i:j]
            i=j
        return acceptable
    return out
...
def repeated_delete(texts,min_cluster_size=1,whiteout=True):
    ...
    @param min_cluster_size: minimum size of repeated group
    @param drop_timestamps: if true, drops timestamps before evaluation
    @param whiteout: if true, doesnt delete the list index but replaces it with empty string
    """
    repeats = repeated_find(texts,min_cluster_size)
    if not repeats: return texts
    ...
    # if whiteout, replace with whitespace
        ...
        return [texts[i] if i not in repeats else '' for i in range(len(texts))]
