...
def plot_coefficients(classifier, variables, n=50, bottom=False):
    ...
    if bottom:
        ...
        plt.title("Top " + str(int(n)) + " Variable Coefficients")
    plt.show()
...
def plot_confidences(text, ensemble, vocab, windowsize, step=5):
    ...
    for classifier in classifier_names:
        if ensemble[classifier]["classifier"]:
            peak_prob.append(
                max(
                    ensemble[classifier]["classifier"].predict_proba(vectors)[
                        :, 1
                    ]
                )
            )
            ...
            peak_prob.append(0)
    peak_prob, classifier_names = (
        list(t) for t in zip(*sorted(zip(peak_prob, classifier_names)))
    )
    ...
    plt.figure(
        figsize=(
            5,
            ((np.sum(len(classifier_names)) + 1) + (2) * spacing) * barwidth,
        )
    )
    plt.barh(
        np.arange(len(classifier_names)), peak_prob, align="center", alpha=0.5
    )
    ...
    plt.yticks(
        np.arange(len(classifier_names)),
        [ensemble[i]["description"] + " " + str(i) for i in classifier_names],
    )
    plt.grid(linestyle="dashed", axis="x")
    plt.show()
...
def print_findings(
    text, classifier, variables, windowsize=100, step=50, threshold=0.5
):
    """
    ...
    pages, token_s, token_e, vectors = zip(
        *extraction.build_convolutions(
            text, windowsize, step=step, metadata=True
        )
    )
    ...
    if isinstance(classifier, dict):
        for i in classifier:
            if not classifier[i]["classifier"]:
                continue
    ...
    # or just a single if provided
        ...
        for i in range(len(prob)):
            if prob[i] >= threshold:
                print("Conf: " + format(prob[i], ".2f") + "\t" + vectors[i])
...
def plot_performance(y_true, y_score, datapoint=False, title=None):
    ...
    if datapoint:
        plt.plot(
            datapoint[0], datapoint[1], marker="x", markersize=5, color="red"
        )
    plt.xlabel("Recall")
    ...
    if datapoint:
        plt.plot((0, 1), (datapoint[1], datapoint[1]), color="red")
...
def plot_timeline(text, classifier, vocab, windowsize, step=5):
    ...
    if isinstance(text, list):
        text = "\n".join(text)
    text = preprocessing.preprocess(text)
    pages, word_s, word_e, windows = zip(
        *extraction.build_convolutions(
            text, windowsize, step=step, metadata=True
        )
    )
    ...
    if isinstance(classifier, dict):
        ...
        for i in classifier:
            ...
            plt.title(
                "Confidence of Detection of '" + str(i) + "' Across Document"
            )
            plt.ylabel("Confidence of Detection")
            plt.xlabel("Token Index")
            plt.ylim((0, 1))
            plt.xlim((-0.5, len(windows) - 0.5))
