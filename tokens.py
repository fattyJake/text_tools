...
###############################################################################
# MODULE: vocab_tools
# this is a repo of tools to evaluate and jump between tokens
# NOTE: tokens are different from words in that the can be alphanumeric
# 
...
def fullword(text,index):
    textlen = len(text)
    if index==None or not text[index].isalnum():
        return None
    start = index
    end = index
    
    # step backwards
    while start-1>0 and text[start-1].isalnum(): start-=1
        
    # step forwards
    while end+1<textlen and text[end+1].isalnum(): end+=1
...
def nextwordindex(text,index):
    ...
    if text[index].isalnum():
        while index<textlen and text[index].isalnum():
            index+=1
    else: index+=1
    if index>=textlen or text[index] in punct: return None
    
    # step to start of next word
    while index<textlen and text[index] not in punct:
        if index>=textlen or text[index] in punct: return None
        if text[index].isalnum(): return index
        index+=1
    return None
...
def prevwordindex(text,index):
    ...
    if index<0 or text[index] in punct: return None

    # find end of previous word
    while text[index] not in punct and not text[index].isalnum():
        if index<0 or text[index] in punct: return None
        index-=1
    if index<0 or text[index] in punct: return None
    ...
    while index>=0 and text[index].isalnum():
        index-=1
        if index==-1: return 0
        if not text[index].isalnum(): return index+1
    return None
...
def wordstartindex(text,index):
    if index==None or not text[index].isalnum(): return None
