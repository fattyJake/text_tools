...
def fullword(text,index):
    ...
    # step backwards
    while start-1>0 and text[start-1].isalpha(): start-=1
        
    # step forwards
    while end+1<textlen and text[end+1].isalpha(): end+=1
...
def nextwordindex(text,index):
    ...
    while index<textlen and text[index] not in punct:
        if index>=textlen or text[index] in punct: return None
        if text[index].isalpha(): return index
        index+=1
    return None
...
def prevwordindex(text,index):
    ...
    while index>=0 and text[index].isalpha():
        index-=1
        if index==-1: return 0
        if not text[index].isalpha(): return index+1
    return None
...
def wordstartindex(text,index):
    if index==None or not text[index].isalpha(): return None
