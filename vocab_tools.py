...
def vocab_regex(vocab,ignorecase=False):
    ...
    """
    if isinstance(vocab,str): vocab = [vocab]
    assert len(vocab)==len(list(set(vocab))),'Error: repeat phrase found in vocabulary'
    vocab = copy(vocab)
    vocab.sort(key=len,reverse=True)
    vocab = r'\b(' + r'|'.join(vocab) + r')\b'
    vocab = re.sub(r"""\-""",r"""\-""",vocab)
    vocab = re.sub(r"""\.""",r"""\.""",vocab)
    if ignorecase==True: return re.compile(vocab,re.IGNORECASE)
...
def build_vocab(text,multigram=False,vocab_init=[]):
    ...
    abbrevs = [i.lower() for i in re.findall(r'\b[A-Z]{3,}\b',text)]     # pulls out abbrevs and add them to delimiter list
    ...
    vocab = list(set(abbrevs + re.findall(r'\b[a-z]{2,40}\b',text) + vocab_init))
...
def vocab_preprocess(vocab,max_length=40):
    ...
    """
    if isinstance(vocab,str): vocab = vocab.splitlines()
    ...
    stopwords.sort(key=len,reverse=True)
    stopwords = r'\b' + r'\b|\b'.join(stopwords) + r'\b'
    regex_stops = re.compile(stopwords + r'|\b[a-z]*\d+[a-z]*\b|\%|\~|\||\\|!|\*|\"|\'|\(|\)|\+|\,|\.|\|/`|\[|\]|\^|\;|\:|\{|\}|\<|\>|\?|\n|\t|\r|\f')
