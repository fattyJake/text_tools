...
def highlight_by_token(text,locs,color='#6cbbf7',title='Test'):
    ...
    # get token dict
    text = text.strip()
    delimiters = [0] + [m.start()+1 for m in re.finditer(r'\s+', text)] + [len(text)]
    ...
    if len(locs[0]) == 2:
        for j in locs: text = text[: token_dict[j[0]-1]] + """<span class="tooltip" style="background-color: """+color+"""">""" + text[token_dict[j[0]-1]: token_dict[j[1]]] + """</span>""" + text[token_dict[j[1]]:]
    if len(locs[0]) == 3:
        for j in range(len(locs)):
            locs[j][2] = ', '.join(sorted(list(set(locs[j][2].split(', ')))))
            
        for j in locs: text = text[: token_dict[j[0]-1]] + """<span class="tooltip" style="background-color: """+color+"""">""" + text[token_dict[j[0]-1]: token_dict[j[1]]] + """<span class="tooltiptext">"""+str(j[2]).upper()+"""</span></span>""" + text[token_dict[j[1]]:]
    css = """<!DOCTYPE html>
        <html>
        <style>
            /* Tooltip container */
...
def _cluster_highlights(locs):
    ...
    for i in range(len(locs)-2,-1,-1):
        ...
        if locs[i][0] <= locs[i+1][1]:
            ...
            if len(locs[i]) == 3:
                locs[i][2] = ', '.join(list(set([locs[i][2].strip(), locs[i+1][2].strip()])))
            locs.pop(i+1)
    return locs
