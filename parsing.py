...
def build_document_from_dir(directory=None):
	...
	for f in os.listdir(directory):
		if f[-4:] == '.txt': doc.append(open(f,'r').read())
	return doc
...
def extract_text(fileORurl):
    ...
    if is_url(fileORurl):
        try: 
            with urllib.request.urlopen(fileORurl) as response: text = response.read().decode('utf-8')
    ...
    try:
        ...
        assert text!=None or text!='', 'Text could not be parsed. Aborting.'
    except:
...
def is_url(url):
    ...
    """
    re_url = re.compile(r'((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w_-]*)?\??(?:[-\+=&;%@.\w_]*)#?(?:[\w]*))?)')
    url = url.strip()
    if re_url.match(url): return True
    else: return False
...
def audio_extract(filepath):
    ...
    with sr.AudioFile(filepath) as source:
        ...
        stamps  = [(preprocessing.strip_parenthesized(seg.word), seg.start_frame/framerate) for seg in decoder.seg()]
        stamps  = stamps[1:-1]
    return stamps